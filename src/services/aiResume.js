...
const experienceSectionConfig = {
  work: {
    ...
    outputKey: "work",
    instruction:
      "生成 1 到 3 条工作经历。只使用素材中明确属于正式工作、全职、公司任职或在职交付的经历。若素材超过 3 条，必须按目标岗位相关性、职责深度、交付价值和可信度择优取舍。JSON 字段保持固定，但字段内容必须是自然简历文本，不要在内容里写“背景：”“重点：”“行动：”“技术栈：”等标签。",
  },
  internships: {
    label: "实习经历",
    outputKey: "internships",
    instruction:
      "生成 1 到 3 条实习经历。只使用素材中明确属于实习或实习生岗位的经历。若素材超过 3 条，必须按目标岗位相关性、职责深度、交付价值和可信度择优取舍。JSON 字段保持固定，但字段内容必须是自然简历文本，不要在内容里写“背景：”“重点：”“行动：”“技术栈：”等标签。",
  },
  projects: {
    label: "项目经历",
    outputKey: "projects",
    instruction:
      "生成 1 到 3 条项目经历。使用素材中的项目、系统建设、产品开发、课程项目等经历，不要把明确的正式工作或实习重复写入项目经历。若素材超过 3 条，必须按目标岗位相关性、技术含量、个人贡献、完成度和可信度择优取舍。JSON 字段保持固定，但字段内容必须是自然简历文本，不要在内容里写“项目背景：”“项目难点：”“行动：”“技术栈：”等标签。",
  },
};
    ...
    messages: [
      {
        role: "system",
        content:
          `你是中文${config.label}简历写作助手。只能基于用户提供的目标岗位、HR岗位描述和经历素材生成${config.label}。经历条目最多 3 条，素材多时必须主动取舍，优先保留最能证明岗位匹配度、个人贡献和交付价值的经历。写作必须像真实经历复盘，不能像为了迎合 JD 而堆关键词。不要复刻 JD 的招聘话术或门槛表达，例如“至少会”“优先”“加分项”“任职要求”“岗位职责”“需要具备”“熟练掌握者优先”。相关技能字段只列实际技术、框架、工具名称，不写能力门槛句。不要使用任何现有模板示例内容，不要编造不存在的项目、公司、时间、量化结果。为了适配一页简历，每条行动最多 2 条，每条行动一句话。不要输出分析过程，只返回 JSON。`,
      },
      {
        role: "user",
        content: JSON.stringify({
